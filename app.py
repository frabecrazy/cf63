...
def show_main():
    ...
    if st.button("➕ Add Device"):
        ...
        for k in st.session_state.device_expanders:
            st.session_state.device_expanders[k] = False  # Close all others
        st.session_state.device_expanders[new_id] = True  # Open new one
    ...
    for device_id in st.session_state.device_list:
        ...
        if device_id not in st.session_state.device_expanders:
            st.session_state.device_expanders[device_id] = True

        with st.expander(f"{base_device}", expanded=st.session_state.device_expanders[device_id]):
            col1, col2 = st.columns([2, 2])
            with col1:
                st.markdown("""<div style='margin-bottom:-10px'><strong>Ownership</strong><br/>
                <span style='font-size:12px; color:gray'>Is this device used only by you or shared?</span></div>""", unsafe_allow_html=True)
                shared = st.selectbox("", ["Personal", "Shared"], index=["Personal", "Shared"].index(prev["shared"]), key=f"{device_id}_shared")

                st.markdown("""<div style='margin-bottom:-10px'><strong>Condition</strong><br/>
                <span style='font-size:12px; color:gray'>Was the device new or used when you got it?</span></div>""", unsafe_allow_html=True)
                used = st.selectbox("", ["New", "Used"], index=["New", "Used"].index(prev["used"]), key=f"{device_id}_used")
            with col2:
                st.markdown("""<div style='margin-bottom:-10px'><strong>Device's lifespan</strong><br/>
                <span style='font-size:12px; color:gray'>How many years you plan to use the device in total</span></div>""", unsafe_allow_html=True)
                years = st.number_input("", 0.5, 20.0, step=0.5, format="%.1f", key=f"{device_id}_years")

                st.markdown("""<div style='margin-bottom:-10px'><strong>End-of-life behavior</strong><br/>
                <span style='font-size:12px; color:gray'>What do you usually do when the device reaches its end of life?</span></div>""", unsafe_allow_html=True)
            ...
            adj_years = (
                years if used == "New" and shared == "Personal" else
                years + (years / 2) if used == "Used" and shared == "Personal" else
                years * 3 if used == "New" and shared == "Shared" else
                years * 4.5
            )
